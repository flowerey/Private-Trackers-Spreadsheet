...
// Load existing trackers.json to merge duplicates
const mainJson = JSON.parse(fs.readFileSync('trackers.json', 'utf-8'));
let trackers2Json = { trackers: [] };
if (fs.existsSync('trackers2.json')) {
  trackers2Json = JSON.parse(fs.readFileSync('trackers2.json', 'utf-8'));
}
...
const trackers2Map = new Map();
trackers2Json.trackers.forEach(tr => {
  trackers2Map.set(normalizeName(tr.name), tr);
});

// Custom ignore list
const customIgnore = [
  'beyond-hd (oneurl)','efecto doppler','empornium2fa','hdbits (api)','hon3y hd','insane tracker',
  'jptv','m-team - tp','mteamtp2fa','racing4everyone (r4e)','snowpt','the geeks','the place',
  'the vault','totheglorycookie','xwtorrents'
];

// Type cleanup mapping
const cleanTypes = {
  "audioaudiobook":"Audiobooks","audioforeign":"Audio","audiolossless":"Audio",
  "audiomp3":"Audio","audioother":"Audio","audiovideo":"Audio","bookscomics":"Comics",
  "booksebook":"Books","booksforeign":"Books","booksmagazines":"Magazines","booksmags":"Magazines",
  "booksother":"Books","bookstechnical":"Books","console3ds":"Console","consolends":"Console",
  "consoleother":"Console","consoleps3":"Console","consoleps4":"Console","consolepsp":"Console",
  "consolepsvita":"Console","consolewii":"Console","consolewiiu":"Console","consolewiiwarevc":"Console",
  "consolexbox":"Console","consolexbox360":"Console","consolexbox360dlc":"Console","consolexboxone":"Console",
  "movies3d":"Movies","moviesbluray":"Movies","moviesdvd":"Movies","moviesforeign":"Movies","movieshd":"Movies",
  "moviesother":"Movies","moviessd":"Movies","moviesuhd":"Movies","movieswebdl":"Movies","other":"General",
  "otherhashed":"General","othermisc":"General","pc0day":"PC","pcgames":"Games","pciso":"PC",
  "pcmac":"Mac Software","pcmobileandroid":"Android","pcmobileios":"iOS","pcmobileother":"Phone",
  "pcphoneandroid":"Android","pcphoneios":"iOS","pcphoneother":"Phone","tvanime":"Anime",
  "tvdocumentary":"TV","tvforeign":"TV","tvhd":"TV","tvother":"TV","tvsd":"TV","tvsport":"Sports",
  "tvuhd":"TV","tvwebdl":"TV","xxxdvd":"XXX","xxximageset":"XXX","xxxother":"XXX","xxxpack":"XXX",
  "xxxpacks":"XXX","xxxsd":"XXX","xxxuhd":"XXX","xxxwmv":"XXX","xxxx264":"XXX","xxxxvid":"XXX"
};
...
async function githubJson(url) { const res = await fetchWithRetry(url,{headers:authHeaders()}); return res.json(); }
...
async function main(){
  try{
    ...
    const definitions = tree.filter(t=>t.type==='blob'&&t.path.startsWith('src/Jackett.Common/Definitions/')&&(t.path.endsWith('.yml')||t.path.endsWith('.yaml')));
    const indexers = tree.filter(t=>t.type==='blob'&&t.path.startsWith('src/Jackett.Common/Indexers/')&&t.path.endsWith('.cs'));
    ...
    // --- MERGE WITH trackers2.json ---
    for(const tr of trackers.trackers){
      if(!tr.name || !tr.name.trim()) continue;
      const normName = normalizeName(tr.name);
      if(trackers2Map.has(normName)){
        const existing = trackers2Map.get(normName);
        if(!existing.description && tr.description) existing.description=tr.description;
        if(!existing.type && tr.type) existing.type=tr.type;
      }else{
        trackers2Json.trackers.push(tr);
        trackers2Map.set(normName,tr);
      }
    }
    ...
    fs.writeFileSync('trackers2.json',JSON.stringify(trackers2Json,null,2)+'\n');
    console.log('trackers2.json merged and saved successfully. Total trackers: '+trackers2Json.trackers.length);
